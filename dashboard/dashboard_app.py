...
CHART_INFO = {
    "kpi": (
        "**📈 How to read these numbers:**  \n"
        "- **Baseline accuracy** = model performance on normal, unmodified inputs.  \n"
        "- **Under attack** = performance after adversarial examples are fed in.  \n"
        "- **Accuracy drop** = the difference. Bigger drop = more vulnerable model.  \n"
        "- **Mean L2 perturbation** = average magnitude of changes made to inputs. "
        "Small L2 = attack is subtle and hard to detect."
    ),
    "bar": (
        "**📊 What the bars mean:**  \n"
        "- 🟢 **Green** = baseline accuracy on clean inputs.  \n"
        "- 🔴 **Red** = accuracy under attack.  \n"
        "- **Coloured bar** = accuracy after applying the selected defence.  \n"
        "A good defence pushes the coloured bar close to the green bar."
    ),
    "features": (
        "**🔥 What the feature perturbation chart shows:**  \n"
        "Each bar is one network traffic feature (like `src_bytes`, `count`, etc.). "
        "Bar length = how much that feature was changed on average.  \n"
        "When a defence is active, this chart shows perturbation *after* the defence — "
        "you can see how much it cleaned up the adversarial signal."
    ),
    "confusion": (
        "**🧮 How to read the confusion matrices:**  \n"
        "Rows = actual labels. Columns = predicted labels.  \n"
        "- **Top-left** = correctly classified Normal traffic.  \n"
        "- **Bottom-right** = correctly detected Attacks.  \n"
        "- **Top-right** = Normal traffic flagged as Attack (false alarms).  \n"
        "- **Bottom-left** = Attacks that slipped through undetected — **dangerous!**  \n"
        "A successful attack grows the bottom-left number. A good defence shrinks it back."
    ),
    "sweep": (
        "**📉 How to read the ε sweep chart:**  \n"
        "- 🟢 **Green** = clean accuracy. Flat — ε doesn't affect clean inputs.  \n"
        "- 🔴 **Red dashed** = adversarial accuracy. Drops as ε increases.  \n"
        "- **Dotted line** (when defence active) = accuracy after defence at each ε.  \n"
        "A strong defence keeps the dotted line close to the green line across all ε values."
    ),
    "table": (
        "**🔍 What the sample table shows:**  \n"
        "Each row is one network connection from the test set.  \n"
        "- **Flipped?** ⚠️ = the attack changed the model's answer for this sample.  \n"
        "- **Recovered?** (when defence active) = did the defence fix the flipped prediction?  \n"
        "Highlighted rows (orange) = the model was fooled."
    ),
    "summary": (
        "**🏆 Defence summary:**  \n"
        "- **Accuracy lost** = what the attack took away.  \n"
        "- **Accuracy recovered** = how much the defence gave back. 100% = perfect defence.  \n"
        "The percentage is *of the lost accuracy*, not absolute accuracy."
    ),
}
...
c3.metric("Accuracy drop",        f"{drop:.3f}",
          delta=f"▼ {drop/max(clean_acc,1e-9)*100:.1f}%", delta_color="inverse")
c4.metric("Mean L2 perturbation", f"{l2:.3f}",
          help="Average size of changes made to inputs.")
...
# ══════════════════════════════════════════════════════════════════════════════
# SECTION 2 — Bar chart + Feature perturbation
# ══════════════════════════════════════════════════════════════════════════════
col_left, col_right = st.columns(2)
...
with col_left:
    ...
    if defended_acc is not None:
        ...
        bar_values.append(defended_acc)
        bar_colors.append(def_color)
    st.plotly_chart(bar_chart(bar_labels, bar_values, bar_colors), width="stretch")
    explain("bar")
...
if x_def is not None:
    cm1, cm2, cm3 = st.columns(3)
    with cm1:
        st.plotly_chart(cm_fig(y_sub, clean_preds, "✅ Clean input"), width="stretch")
    with cm2:
        st.plotly_chart(cm_fig(y_sub, adv_preds, f"🔴 Under {attack_type}"), width="stretch")
    with cm3:
        def_preds = def_model.predict(x_def)
        st.plotly_chart(cm_fig(y_sub, def_preds, f"🔵 After {defense_type}"), width="stretch")
else:
    cm1, cm2 = st.columns(2)
    with cm1:
        st.plotly_chart(cm_fig(y_sub, clean_preds, "✅ Clean input"), width="stretch")
...
sweep_def = sweep_def_label = sweep_def_color = None
if defense_type == "Feature Squeezing":
    def atk_key(e):
        return ("fgsm", e) if attack_type in ("FGSM", "Feature Perturbation") else ("pgd", e, pgd_iter_choice)
    sweep_def       = [acc(model, sq_cache[atk_key(e)][:n_samples], y_sub) for e in EPS_SWEEP]
    ...
    def adv_at_e(e):
        return (adv_cache["fgsm"][e][:n_samples] if attack_type in ("FGSM", "Feature Perturbation")
                else adv_cache["pgd"][e][pgd_iter_choice][:n_samples])
    sweep_def       = [acc(hardened_model, adv_at_e(e), y_sub) for e in EPS_SWEEP]
    sweep_def_label = "Hardened model"
    sweep_def_color = PALETTE["hardened"]
...
if x_def is not None:
    def_p10 = def_model.predict(x_def[:10])
    tbl[defended_label] = ["Attack" if v else "Normal" for v in def_p10]
    tbl["Recovered?"] = [
        "✅ Yes" if (ap != cp and dp == cp) else
        "❌ No"  if (ap != cp and dp != cp) else "—"
        for cp, ap, dp in zip(clean_p10, adv_p10, def_p10)
...
def _hl(row):
    return (["background-color: rgba(216,90,48,0.15)"] * len(row)
            if row.get("Flipped?", "") == "⚠️ Yes" else [""] * len(row))

st.dataframe(df_tbl.style.apply(_hl, axis=1), width="stretch")
explain("table")
st.markdown("---")

# ══════════════════════════════════════════════════════════════════════════════
# SECTION 6 — Defence summary
# ══════════════════════════════════════════════════════════════════════════════
if defended_acc is not None:
    st.subheader("🏆 Defence Summary")
    recovery     = defended_acc - adv_acc
    ...
    if recovery_pct >= 90:
        ...
        st.error(
            f"**Weak defence against this attack.** {defense_type} only recovered "
            f"**{recovery_pct:.0f}%** of the lost accuracy at ε={eps}. "
            "Try a different defence or reduce ε."
        )
    explain("summary")
